...
où on a rajouté un prior gaussien sur le champ $u$. Par facilité d'écriture on suppose que cette norme s'applique sur le champ discrétisé sur une grille. Dans ce cas $u \sim \mathcal N(u^f, P^f)$.
Pour déterminer cette loi, on suppose que la distribution est centrée en 0. En effet, notre prior est de penser qu'il n'y a pas de réalignement. On suppose que l'alignement se fait préférentiellement avec le champ de vitesse des membres. Ainsi $P^f$ est pris comme
