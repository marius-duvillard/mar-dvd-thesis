...
\section{Méthode séquentielle - Approche Probabiliste}
...
De ces lois sur les distributions de probabilité peuvent être déduite les estimateurs Bayésiens tels que le MMSE \ref{estimation} accompagné de leur covariance conditionnées aux données

\begin{eqnarray*}
    \hat \bm X_{k+1}^{\mid k} &=& \bE^{\mid k}(\bm X_{k+1}),
    \bm P^{\mid k} &=& \bm V^{\mid k}(\bm X_{k+1}),
...
Introduit par Kalman en 1960 \cite{kalman_new_1960}, le filtre de Kalman est la version linéaire du filtre de bayésien. En supposant les distributions Gaussienne, et le modèle de Chaîne de Markov linéaire \ref{hidden_mc}, \dots
