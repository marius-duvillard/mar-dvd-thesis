...
\begin{algorithm}~\label{algo:part_enkf}
    ...
    \ForEach{$i = 1, \dots, \nens $}{
    $\bm U^a_{ip} = $\approxim{$\mathcal{P}^f_i, \Fcorr$} \tcp*[r]{approximate the analysis fields}
    }
    \Return{$\mathcal{P}^a_i=\{\bm z^f_{ip}, U^a_{ip}\}_{ip = 1}^{N_{a}}, \quad i = 1, \dots, N_{\text{ens}}$ \tcp*[r]{analyse discretizations}}
\end{algorithm}
