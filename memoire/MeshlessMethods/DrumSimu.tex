...
\subsection{Méthodes sans maillage discrètes}
...
L'état est ce qui permet de représenter la connaissance de l'état du système virtuel.

Les méthodes classiques vont utiliser un maillage fixe pour le définir.

En considérant un domaine $\Omega \subset \mathbb R^d$, soit $\mathcal{T} = (\mathcal N, \mathcal E)$ un maillage, où $\mathcal N = \left\{\bm x_i \right\}^M_{i=1}$ est l'ensemble des noeuds, et $\mathcal{E} = \{ K_j \}_{j=1}^{N}$ l'ensemble des éléments.

L'état de la solution \(u_h\) discrétisé peut est définie

\begin{itemize}
    \item sur les nœuds du maillage $\bz = \{ u(\mathbf{x}_i) \}_{i=1}^{M}$, c'est le cas des éléments finis (FEM)
    \item  les éléments
    \item
\end{itemize}


Pour les éléments finis (FEM) par exemple $\mathcal{T}$ est défin
...
% \subsubsection{Méthodes sans maillage continus et méthodes variationnelles}
...
% \subsubsection{Méthodes sans maillage continus et EnKF}
