...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{sources_incertitudes.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{schema d'assimilation.png}
...
% \begin{equation*}
%     p_{\bm z_k}(\bm z) \simeq \sum_{i=1}^N \omega^i_k \delta(\bm x - \bm z_k^i) \quad \text{with} \sum_{i=1}^N \omega^i_k = 1, \quad \omega^i_k > 0.
% \end{equation*}
...
% %     &\simeq& \int p_{\bm X_{k+1}\mid \bm X_{k} = \bm x'}(\bm x) \sum_{i=1}^N \omega^i_k \delta(\bm x' - \bm z_k^i) dx' \\
% %     &\simeq& \sum_{i=1}^N \omega^i_k  \int p_{\bm X_{k+1}\mid \bm X_{k} = \bm x'}(\bm x) \delta(\bm x' - \bm z_k^i) dx' \\
% %     &\simeq& \sum_{i=1}^N \omega^i_k \delta(\bm x - \mathcal M_{k,k+1}(\bm z_k^i) - \bm \eta_{k,k+1}) = \sum_{i=1}^N \omega^i_k \delta(\bm x - \bm z_{k+1}^i).
% % \end{eqnarray*}
...
% \begin{eqnarray*}
%     p_{\bm X_{k+1}}^{\mid k+1}(\bm x) &\propto& p_{\bm Y_{k+1} \mid \bm X_{k+1} = \bm x}^{\mid k}(\bm y)  \sum_{i=1}^N \omega^i_k \delta(\bm x - \bm z_{k+1}^i) \\
%     &\propto& \sum_{i=1}^N  \omega^i_k~p_{\bm Y_{k+1} \mid \bm X_{k+1} = \bm z_{k+1}^i}^{\mid k}(\bm y)\delta(\bm x - \bm z_{k+1}^i)
% \end{eqnarray*}

% ce qui donne

% \begin{equation*}
%     \omega^i_{k+1}  = \frac{\omega^i_k~p_{\bm Y_{k+1} \mid \bm X_{k+1} = \bm z_{k+1}^i}^{\mid k}(\bm y_{k+1}) }{\omega^j_k~\sum_j^N p_{\bm Y_{k+1} \mid \bm X_{k+1} = \bm z_{k+1}^j}^{\mid k}(\bm y_{k+1}) }
% \end{equation*}
...
L'objectif des méthodes variationnelles est de proposer une estimation de l'état ou de sa distribution à partir de la minimisation de fonctions de coût. Pour cela, nous partons de l'estimation du maximum a posteriori.
...
% \begin{eqnarray*}
%     \bm \eta &\sim& \mathcal{N}(\bm 0, P_{k+1}), \quad p(\bm z_{k+1}) = \mathcal{N}(\bm z_{k+1}^f, P_{k+1})\\
%     \bm \varepsilon & \sim & \mathcal N(\bm 0, R), \quad \quad p(\bm y_{k+1}) = \mathcal{N}(\bm g(\bm z_{k+1}^f) , R_{k+1})\\.
% \end{eqnarray*}~où $\bm z_{k+1}^f = \mathcal{M}(\bz_{k})$ l'état prédit par le modèle. Nous nous intéressons maintenant à l'étape de mise à jour à l'instant $k+1$, l'indice temporel sera implicite pour le reste de la subsection.
...
Cependant, en conservant la matrice de rang faible $\bm P^f$ il est possible d'exprimer la solution dans l'espace de perturbations. En effet, $\bm P^f$ étant symétrique, $\text{Span}(\bm P^f)$ est bijectif avec l'espace généré par les colonnes de $\annomX$.
