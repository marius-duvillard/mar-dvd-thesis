...
\begin{eqnarray*}
    ...
    \bm \bm \varepsilon & \sim & \mathcal N(\bm 0, R), \quad \quad p(\bm x) = \mathcal{N}(\bm , R)\\.
\end{eqnarray*}

Ainsi la distribution a posteriori peut être réécrite comme

\begin{equation*}
    p(\bm x \mid \bm y) \propto \exp\left(- \mathcal{\lambda(\bm x)}\right),
\end{equation*}
...
De plus, le logarithme étant une fonction strictement croissante, la maximisation de la posterior est équivalente à minimiser \lambda. D'où la nouvelle expression de $\bm x_{\text{MAP}}$

\begin{equation*}
    \bm x_{\text{MAP}} = \argmin_{\bm x} \lambda(\bm x).
\end{equation*}
...
\begin{equation*}
    \nabla  \lambda(\bm x_{\text{MAP}}) = \bm 0.
\end{equation*}
