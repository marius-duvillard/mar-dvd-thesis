...
\begin{equation*}
    \nabla_\bx \mathcal L(\bx^a) = \bP^{-1} (\bx^a - \bx^f) + \nabla_{\bx^a} \bm h (\bx^a) \bR^{-1}(\bg(\bx^a) - \bm d) = \bm 0.
\end{equation*}
...
\section{Méthode 3DVar et équivalence avec la mise à jour de Kalman}

La méthode 3DVar est un cas particulier de l'équation précédente qui permet d'avoir une solution linéaire au problème. Elle se place dans le cas où la fonction d'observation est linéaire $\bm h(\bx) = \bH \bx$.
...
\begin{equation*}
    \bx^a = \bx^f + (\bP^{-1} + \bH^T \bR^{-1} \bH)^{-1} \bH^T \bR^{-1} (\bd - \bH \bz^f),
\end{equation*}~qui est l'expression de la mise à jour dans l'espace d'état.Il peut être coûteux d'inverser la matrice $(\bP^{-1} + \bH^T \bR^{-1} \bH)^{-1}$ si l'espace d'état est de grande dimension. A l'aide de la formule de Sherman-Morisson-Woodbury, la mise à jour peut être réécrite dans l'espace de mesure

\begin{eqnarray*}
    (\bP^{-1} + \bH^T \bR^{-1} \bH)^{-1} = \bP - \bP \bH^T (\bH \bP \bH^T + \bR)^{-1} \bH \bP
\end{eqnarray*}

Ainsi la matrice de covariance d'état $\bP$ ne necessite pas d'être inversé.
...
\subsection{Méthode variationnelle d'ensemble}
